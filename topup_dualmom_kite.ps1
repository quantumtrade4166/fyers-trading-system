...
# A name at its UPPER CIRCUIT is not bought today: its qty is fixed and it is
# bought automatically at 09:25 on the next trading day (pending_circuit.json).
#
...
$log  = Join-Path $root "logs\topup_dualmom_kite.txt"
...
$m = $p.account_margin
Write-Host ""
Write-Host ("   Top-up value     Rs {0,12:N0}" -f $bv)
...
foreach ($c in @($p.circuit_deferred)) { if ($c) { Write-Host ("   NEXT DAY (upper circuit {0}): {1} x{2}" -f $c.upper_circuit, $c.symbol, $c.qty) -ForegroundColor Yellow } }
foreach ($d in @($p.deferred)) { if ($d) { Write-Host ("   does not fit in own cash: {0} x{1}" -f $d.symbol, $d.qty) -ForegroundColor Yellow } }
Write-Host ("   DualMom own cash left after this: Rs {0:N0}" -f $p.own_cash_after)
foreach ($b in @($p.blocked)) { if ($b) { Write-Host "   BLOCKED: $b" -ForegroundColor Red } }
if ($buys.Count -eq 0) { Write-Host "`n>>> Nothing to buy today."; Stop-Transcript | Out-Null; exit 0 }
...
$csv = Join-Path $root ("logs\dualmom_kite_entries_{0}.csv" -f (Get-Date -Format 'yyyyMMdd_HHmm'))
if ($bk.ok) {
...
if ($filled.Count -eq $orders.Count -and -not $dp.halted) {
    Write-Host "`n>>> TOP-UP DONE. Dashboard -> DualMom -> Kite live. Tell Claude ""done""." -ForegroundColor Green
} else {
