...
def get_trail_exit_price(df: pd.DataFrame, date) -> float:
    """Trail stop exit — next trading day open (normal behaviour)."""
...
def get_hwm_exit_price(df: pd.DataFrame, date) -> float:
    """HWM stop exit — today's close (immediate exit at trigger price)."""
    if date in df.index:
...
def run_v12(all_data: dict, all_dates: list,
    ...
    for date in all_dates:
        ...
        # Checked BEFORE trail exits so gradual bleed is caught daily
        ...
        if dd_from_peak <= -HWM_DD_STOP and open_positions:
            ...
            for sym in list(open_positions.keys()):
                ...
                exit_price = get_hwm_exit_price(all_data[sym], date)  # TODAY's close
...
def print_summary(label: str, result: dict):
    ...
    if result["stop_events"]:
        print(f"\n  Stop Events (exit at same-day close):")
...
if __name__ == "__main__":
    ...
    print(f"Portfolio stop  : exit ALL at TODAY'S CLOSE when equity drops {HWM_DD_STOP*100:.0f}% from peak")
    print(f"Market filter   : Nifty 200MA\n")

    print("Running V12a — Dynamic 1% + Nifty + HWM close-exit (no cap)...")
    r12a = run_v12(all_data, all_dates, nifty_above, max_pos_cap=float("inf"))

    print(f"Running V12b — Dynamic 1% + Nifty + HWM close-exit (cap ₹{MAX_POSITION_CAP/1e5:.0f}L)...")
    r12b = run_v12(all_data, all_dates, nifty_above, max_pos_cap=MAX_POSITION_CAP)

    print_summary("V12a — Dynamic 1% + Nifty + HWM stop at close  (no cap)", r12a)
    print_summary(f"V12b — Dynamic 1% + Nifty + HWM stop at close  (cap ₹{MAX_POSITION_CAP/1e5:.0f}L)", r12b)

    out = Path(__file__).parent / "results"
    out.mkdir(exist_ok=True)
    for tag, r in [("v12a", r12a), ("v12b", r12b)]:
    ...
    prev = {
        ...
        "V11b Dynamic 1% + Nifty + HWM (next open)":"equity_55day_v11b.csv",
    }
    ...
    for name, r in [
        ("V12a Dynamic 1% + Nifty + HWM close (no cap)",    r12a),
        (f"V12b Dynamic 1% + Nifty + HWM close (cap ₹{MAX_POSITION_CAP/1e5:.0f}L)", r12b),
    ]:
        eq    = r["equity"]["equity"]
        years = (eq.index[-1] - eq.index[0]).days / 365.25
        cagr  = ((eq.iloc[-1] / STARTING_CAPITAL) ** (1/years) - 1) * 100
    ...
    fig.suptitle("55-Day Breakout V12 — HWM Stop at Same-Day Close (Live Trading Simulation)",
                 fontsize=13, fontweight="bold")

    for r, label, ec, dc, ax_eq, ax_dd in [
        (r12a, "V12a — No cap",        "#2196F3", "#F44336", axes[0][0], axes[1][0]),
        (r12b, "V12b — Cap ₹2L/pos",   "#4CAF50", "#FF9800", axes[0][1], axes[1][1]),
    ]:
        eq = r["equity"]["equity"]
        pk = eq.cummax()
        dd = (eq - pk) / pk * 100
        ...
        for ev in r["stop_events"]:
            ax_eq.axvline(pd.Timestamp(ev["date"]), color="red", linewidth=0.9, alpha=0.7)
        ...
        ax_dd.axhline(-15, color="orange", linestyle=":", linewidth=1.2, label="Stop -15%")
        ax_dd.grid(True, alpha=0.3); ax_dd.legend(fontsize=8)

    plt.tight_layout()
    chart_path = out / "equity_drawdown_v12.png"
