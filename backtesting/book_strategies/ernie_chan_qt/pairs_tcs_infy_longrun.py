# ============================================================
# pairs_tcs_infy_longrun.py
#
# TCS / INFY Pairs Trading — 11-Year Backtest (2015–2026)
#
# Data sources:
#   2015-01-01 → 2024-05-27 : Yahoo Finance (yfinance, Adj Close)
#   2024-05-28 → 2026-05-27 : Fyers 5-min OHLCV → resampled daily
#
# On first run: downloads Yahoo data, caches to parquet.
...
TICKER_A    = "TCS.NS"          # Yahoo Finance ticker
TICKER_B    = "INFY.NS"
NAME_A      = "TCS"
NAME_B      = "INFY"

YF_START    = "2015-01-01"
YF_END      = "2024-05-27"      # day before Fyers data begins

LOT_A       = 150               # TCS   shares per lot
LOT_B       = 400               # INFY  shares per lot
N_LOTS      = 1
...
# Annual P&L stop — halt new entries for rest of calendar year
# if cumulative year loss exceeds this. Set to None to disable.
ANNUAL_STOP = 40_000    # ₹40,000 annual loss cap

MAX_HALFLIFE_DAYS = 120   # relaxed for long-run: global HL measures structural drift
...
EQUITY_CURVE_PATH = RESULTS_DIR / "equity_curve_pairs_tcs_infy_longrun.png"
TRADES_PATH       = RESULTS_DIR / "trades_pairs_tcs_infy_longrun.csv"
SPREAD_PLOT_PATH  = RESULTS_DIR / "spread_zscore_tcs_infy_longrun.png"
...
def fetch_yahoo(ticker: str, start: str, end: str) -> pd.Series:
    """Download adjusted daily close from Yahoo Finance."""
    ...
    close.name  = ticker.split(".")[0]   # "TCS" or "INFY"
    return close.dropna()


def load_yahoo_panel() -> pd.DataFrame:
    """Load historical daily data from cache or Yahoo Finance."""
...
def load_fyers_panel(loader: DataLoader) -> pd.DataFrame:
    """Load Fyers 5-min data, resample to daily close."""
...
def load_combined_panel(loader: DataLoader) -> pd.DataFrame:
    """
    Stitch Yahoo Finance (2015–2024) with Fyers (2024–2026).
    Yahoo uses Adjusted Close; Fyers uses raw close.
    At the join point (May 2024) prices should align closely
    since TCS/INFY had no stock splits in this period.
    """
    ...
    # Combine: Yahoo up to YF_END, Fyers from YF_END+1 onwards
    # Remove overlap — Fyers data takes priority for recent dates
    ...
    # Use actual last Yahoo date (may differ from cutoff if market was closed)
    ...
    if not before.empty and not after.empty:
        ...
        print("  (No overlap at join point — check cutoff dates)")
    print()
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 2 — COINTEGRATION SUITE  (same as v1)
# ─────────────────────────────────────────────────────────────────────────────
...
def calc_trade_pnl(position, entry_pa, entry_pb, ppa, ppb, qty_a, qty_b):
    """Compute gross P&L, cost, net for a closing trade."""
...
def simulate(signals: pd.DataFrame) -> tuple:
    ...
    for t in range(n):
        ...
        # ── New year: reset annual tracker ────────────────────────────────────
        if bar_year != current_year:
            if annual_halted:
                print(f"    → {current_year} annual stop lifted. "
                      f"Resuming {bar_year}.")
        ...
        if position != 0:
            ...
            if abs(z) < EXIT_Z:
                ...
                exit_reason = "annual-stop"   # force-close open trade
            ...
            if exit_reason:
                ...
                # Check if this trade pushed us past the annual stop
        ...
        # ── Entry — blocked if annual stop is active ───────────────────────────
...
def save_spread_plot(signals: pd.DataFrame):
    ...
    ax1.legend(l1+l2, lb1+lb2, loc="upper left", fontsize=8); ax1.grid(True, alpha=0.2)

    ax2 = axes[1]
    ax2.plot(dates, signals["spread"], color="#4A148C", lw=0.8, label="Rolling Spread")
    ax2.axhline(0, color="#9E9E9E", ls="--", lw=0.7)
    ax2.set_ylabel("Spread ₹", fontsize=9); ax2.legend(loc="upper left", fontsize=8)
    ...
    ax3.set_ylabel("Z-score", fontsize=9); ax3.set_xlabel("Date", fontsize=9)
    ax3.xaxis.set_major_formatter(mdates.DateFormatter("%Y"));
    ax3.xaxis.set_major_locator(mdates.YearLocator())
    ax3.legend(loc="upper left", fontsize=8); ax3.grid(True, alpha=0.2)

    plt.xticks(rotation=30, ha="right"); plt.tight_layout()
    plt.savefig(SPREAD_PLOT_PATH, dpi=150, bbox_inches="tight"); plt.close()
    print(f"  Spread plot    → {SPREAD_PLOT_PATH}")


def save_equity_plot(daily_pnl, equity, drawdown, m):
    # Mark year boundaries for context
    ...
    ax1.legend(loc="upper left", fontsize=8); ax1.grid(True, alpha=0.25)

    ax2.fill_between(dates, drawdown, 0, color="#E53935", alpha=0.55)
    ax2.set_ylabel("Drawdown %", fontsize=9); ax2.set_xlabel("Date", fontsize=9)
    ax2.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
    ax2.xaxis.set_major_locator(mdates.YearLocator())
    ax2.grid(True, alpha=0.25)

    plt.xticks(rotation=30, ha="right"); plt.tight_layout()
    plt.savefig(EQUITY_CURVE_PATH, dpi=150, bbox_inches="tight"); plt.close()
...
def main():
    ...
    annual_stop_exits = (trade_df["exit_reason"] == "annual-stop").sum() if not trade_df.empty else 0
    print(f"  Stop-losses hit   : {m['stops_hit']} / {m['total_trades']}")
    print(f"  Annual stop exits : {annual_stop_exits} / {m['total_trades']}")
    print(f"  Annual P&L cap    : ₹{ANNUAL_STOP:,.0f}" if ANNUAL_STOP else "  Annual P&L cap    : disabled")
