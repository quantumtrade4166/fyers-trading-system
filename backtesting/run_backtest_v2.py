# ============================================================
# backtesting/run_backtest_v2.py
#
# BB Reversion Strategy — V2 (filtered)
#
# Changes from V1:
#   ✅ Short only
#   ✅ Signal candle range ≤ 0.8%  →  (H-L)/close ≤ 0.008
#   ✅ Max 5 trades per day        →  first 5 entries by time
#   ✅ ₹2,000 risk per trade       →  ₹10,000 daily ÷ 5 slots
#
# Run: python backtesting/run_backtest_v2.py
# ============================================================
...
CONFIG = {
    **DEFAULT_CONFIG,
    ...
    "risk_per_trade":        2_000,       # ₹2,000 per trade (₹10k daily ÷ 5 slots)
    "max_position_pct":      0.20,
    "exit_time":             "15:00",
    # ── V2 filters ────────────────────────────────────────────────────────────
    "direction":             "short",     # short only
    "max_signal_candle_pct": 0.008,       # signal candle (H-L)/close ≤ 0.8%
    "max_trades_per_day":    5,           # max 5 entries per day across all stocks
}

RESULTS_DIR = Path(r"G:\Trading Backtesting\results")
RESULTS_DIR.mkdir(exist_ok=True)
RUN_TS    = datetime.now().strftime("%Y%m%d_%H%M")
TRADES_CSV  = RESULTS_DIR / f"bb_reversion_v2_trades_{RUN_TS}.csv"
SUMMARY_CSV = RESULTS_DIR / f"bb_reversion_v2_summary_{RUN_TS}.csv"
...
def compute_metrics(trades_df: pd.DataFrame, capital: float) -> dict:
    ...
    roll_mx = cum.cummax()
    max_dd  = ((cum - roll_mx) / roll_mx * 100).min()
    ...
    return {
        ...
        "stops_hit":        reason_counts.get("stop", 0) + reason_counts.get("stop_same_bar", 0),
        "time_exits":       reason_counts.get("time_exit", 0),
...
def print_summary(metrics: dict, sym_df: pd.DataFrame, n_symbols: int):
    line = "=" * 62
    print(f"\n{line}")
    print("  BB REVERSION V2 — SHORT ONLY + FILTERS")
    print(line)
    print(f"  Filters   : short-only | candle ≤0.8% | max 5/day | ₹2k risk")
    ...
    if not sym_df.empty:
        ...
        for _, r in top10.iterrows():
            print(f"  {r['symbol']:<28} {int(r['total_trades']):>6} {r['win_rate_pct']:>5.1f}% {r['total_pnl']:>10,.0f}")
...
def main():
        ...
        try:
            ...
            if n > 0:
                ...
                sym_rows.append({
                    ...
                    "total_pnl":    round(trades_df["pnl"].sum(), 0),
                })
            ...
            if i % 20 == 0 or i == len(symbols):
                print(f"  [{i:>3}/{len(symbols)}] {len(all_trades):>5} raw short signals  "
    ...
    # ── Recompute per-symbol summary on capped trades ─────────────────────────
    sym_capped = (
    ...
    # ── Daily trade count stats ───────────────────────────────────────────────
    daily_counts = capped_df.groupby("entry_date").size()
    print(f"\n  After cap:")
    print(f"    Total trades  : {len(capped_df):,}  (was {len(raw_df):,})")
